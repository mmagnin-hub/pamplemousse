...
public:
    Game() : window(nullptr), renderer(nullptr), font(nullptr), isRunning(true), currentSceneID(0), currentChapterID(0) {}
 ...
 void loadScenes() {
    ...
    void loadChapters() {
        // Ajout du chapitre "Poulpe" avec ses scènes
        Chapter poulpe;
        poulpe.title = "Poulpe";
        poulpe.scenes = scenes; // Toutes les scènes existantes appartiennent au chapitre "Poulpe"
        chapters.push_back(poulpe);

        // // Chargez la musique pour le chapitre "Poulpe"
        // currentMusic = Mix_LoadMUS("../audio/poulpe_theme.mp3");
        // if (!currentMusic) {
        //     std::cerr << "Failed to load music! Mix_Error: " << Mix_GetError() << std::endl;
        // }
        // Mix_PlayMusic(currentMusic, -1); // Musique en boucle
    }
    ...
    void clean() {
        // if (currentMusic) Mix_FreeMusic(currentMusic); // Libérez la musique
        // Mix_CloseAudio();
        TTF_CloseFont(font);
